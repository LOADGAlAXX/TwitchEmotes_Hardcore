...
if [ $# -ge 4 ] ; then
    ...
    # TODO: Resize frames
    magick ${frames[@]} -resize "87x32" -append "$image_appended"
else
    # TODO: Resize frames
    magick import/dump_* -resize "87x32" -background none -append "$image_appended"
fi
...
magick "$image_appended" -background none -gravity North -extent "87x${final_h}" "$image_final"
