...
libwebp/bin/anim_dump -folder import "$image_orig"
...
if [ $# -ge 4 ] ; then
    ...
    # TODO: Resize frames
    magick ${frames[@]} -resize "57x32" -append "$image_appended"
else
    # TODO: Resize frames
    magick import/dump_* -resize "57x32" -background none -gravity center -extent "57x32" -append "$image_appended"
fi
...
magick "$image_appended" -background none -gravity North -extent "57x${final_h}" "$image_final"
